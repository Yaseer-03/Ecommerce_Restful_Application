...
public class UserService implements UserDetailsService {
    ...
    public String registerUser(User user) {
        ...
        if (!validator.isMobileNumberValid(user.getMobileNumber()))
            return "Invalid mobile number: " + user.getMobileNumber();

        // Validating the password
        if (!validator.isPasswordValid(user.getPassword()))
            return "Password must be 8-15 characters long, contain one uppercase letter, and one special character. " + user.getPassword();
